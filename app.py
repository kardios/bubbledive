...
import openai # Keep for existing functionality
import anthropic # Add for new story generation
import json
import re
import time
...
# Initialize the OpenAI client for SparkMap generation
...
def generate_story_with_claude(outline: str, topic: str):
    """Uses the Anthropic Claude API to turn a text outline into a story."""
    try:
        # Best practice: use st.secrets for API keys in deployed apps
        api_key = st.secrets.get("ANTHROPIC_API_KEY")
        if not api_key:
            st.error("ANTHROPIC_API_KEY not found. Please set it in your Streamlit secrets.")
            return None
    ...
    system_prompt = f"""
# Transform Any Outline Into an Engaging Narrative

**Your Mission:** Convert the following outline about '{topic}' into a compelling, immersive narrative that reads like a captivating story rather than a dry report or academic paper.

## Universal Narrative Requirements:

**Hook the Reader Immediately**
- Open with a surprising fact, vivid scene, thought-provoking question, or bold statement from the outline.
- Avoid generic introductions—plunge directly into what's fascinating.
- Make the first paragraph impossible to ignore.

**Create Stakes and Tension**
- Identify what's at risk, what could go wrong, or what's surprising in the outline.
- Build each section with rising tension toward a revelation or climax.
- Frame information as discoveries, conflicts, or turning points.

**Show, Don't Just Tell**
- Use concrete examples, case studies, or specific instances from the outline.
- Include vivid details that help readers visualize concepts.
- Transform abstract ideas into tangible scenarios.
- Use analogies and metaphors to make complex topics accessible.

**Focus on Human Elements**
- Highlight the people behind discoveries, decisions, or developments mentioned in the outline.
- Show how the topic affects real people's lives.

**Structure for Maximum Impact**
- Organize information to build toward the most surprising or important revelations.
- Use cliffhangers and strategic information reveals.

**Use Dynamic, Engaging Language**
- Choose active voice over passive voice.
- Use specific, concrete language instead of abstract terms.

## Universal Engagement Strategies:

**Create Curiosity Gaps:** Present intriguing information, then explain the backstory.
**Use Contrast:** Highlight before/after, expectation vs. reality, conventional wisdom vs. new understanding.
**Build Toward Revelations:** Structure information so each section reveals something surprising.
**Connect to Reader's World:** Show relevance to their daily life, concerns, or interests.
**Include Conflict:** Present opposing viewpoints, challenges, or obstacles that had to be overcome.
**End with Impact:** Conclude with implications, future possibilities, or a call to think differently.

## Final Writing Guidelines:
- Maintain Authority: Support dramatic elements with credible evidence from the outline.
- Be Authentic: Let the inherent fascination of the topic drive the narrative.
- Think Cinematically: Write scenes your readers can visualize and remember.

**Remember:** Your goal is to make readers genuinely excited about '{topic}'. Transform information into revelation, data into drama, and facts into a story worth telling.
"""

    try:
        with st.spinner("Claude is writing a story for you..."):
            message = anthropic_client.messages.create(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": f"Here is the outline you must transform into a narrative:\n\n{outline}"
                    }
                ]
            )
        ...
        st.error(f"An error occurred while generating the story with Claude: {e}")
        return None
...
def create_multilevel_mindmap_html(tree, center_title="Root"):
    ...
    const svg = d3.select("#mindmap").append("svg")
        ...
        .attr("viewBox", `0 0 ${{width}} ${{height}}`).style("background", "#f7faff");
    const container = svg.append("g");
    svg.call(d3.zoom().scaleExtent([0.3, 2.5]).on("zoom", (event) => container.attr("transform", event.transform)));
    const link = container.append("g").selectAll("line").data(links).enter().append("line").attr("stroke", "#b8cfff").attr("stroke-width", 2);
    const node = container.append("g").selectAll("g").data(nodes).enter().append("g").attr("class", "node");
    node.append("circle").attr("r", d => d.id === rootID ? 130 : 75).attr("fill", d => d.id === rootID ? "#93c5fd" : "#fff").attr("stroke", "#528fff").attr("stroke-width", 3)
        .on("mouseover", function(e, d) {{ if(d.tooltip) {{ tooltip.style("opacity", 1).html("<b>" + d.id + "</b><br>" + d.tooltip).style("left", (e.pageX+12)+"px").style("top", (e.pageY-18)+"px"); }} }})
        .on("mousemove", function(e) {{ tooltip.style("left", (e.pageX+12)+"px").style("top", (e.pageY-18)+"px"); }})
        .on("mouseout", function() {{ tooltip.style("opacity", 0); }})
        .on("click", function(e, d) {{
            if (d.id === rootID) return;
            const contextObj = {{ clicked_label: d.id, clicked_tooltip: d.tooltip, parent_label: d.parent, parent_tooltip: d.parent_tooltip, root_label: rootID, root_tooltip: nodes.find(n=>n.id===rootID)?.tooltip || "" }};
            if (d.parent === rootID) {{ contextObj.parent_label = ""; contextObj.parent_tooltip = ""; }}
            window.location.href = `?context=${{encodeURIComponent(JSON.stringify(contextObj))}}`;
        }});
    node.append("text").attr("text-anchor", "middle").style("pointer-events", "none").each(function(d) {{
        const text = d3.select(this), maxChars = 16, maxLines = 4, fontSize = d.id === rootID ? 24 : 20, words = d.id.split(' ');
        let lines = [], current = '';
        words.forEach(word => {{ if ((current + ' ' + word).trim().length > maxChars) {{ lines.push(current.trim()); current = word; }} else {{ current += ' ' + word; }} }});
        if (current.trim()) lines.push(current.trim());
        if (lines.length > maxLines) {{ lines = lines.slice(0, maxLines); lines[maxLines - 1] += "..."; }}
        text.style("font-size", fontSize + "px");
        const startDy = -((lines.length - 1) / 2) * 1.1;
        lines.forEach((line, i) => {{ text.append("tspan").attr("x", 0).attr("dy", i === 0 ? `${{startDy}}em` : "1.1em").text(line); }});
    }});
    const simulation = d3.forceSimulation(nodes)
        .force("link", d3.forceLink(links).id(d => d.id).distance(d => d.source.id === rootID ? 270 : 180))
        .force("charge", d3.forceManyBody().strength(-1400)).force("center", d3.forceCenter(width / 2, height / 2))
        .force("collision", d3.forceCollide().radius(d => (d.id === rootID ? 130 : 75) + 5));
    simulation.on("tick", () => {{
        link.attr("x1", d => d.source.x).attr("y1", d => d.source.y).attr("x2", d => d.target.x).attr("y2", d => d.target.y);
        node.attr("transform", d => `translate(${{d.x}},${{d.y}})`);
    }});
    const tooltip = d3.select("body").append("div").attr("class", "tooltip-glossary");
    </script>
    """
...
def prompt_expand_concept_sparkmap(concept, context=""):
    """Creates the detailed prompt for the OpenAI model to generate the SparkMap."""
    # This function remains unchanged for brevity
...
def condense_bubble_context(clicked_label, clicked_tooltip, parent_label, parent_tooltip, root_label, root_tooltip):
    """Uses the OpenAI model to generate a concise new topic from the context of a clicked bubble."""
    # This function remains unchanged for brevity
    prompt = (
    ...
    # Using the original client for this call
    ...
    try:
        ...
        with st.spinner("Condensing for next dive..."):
            topic = condense_bubble_context(
                context_obj.get("clicked_label", ""), context_obj.get("clicked_tooltip", ""),
                context_obj.get("parent_label", ""), context_obj.get("parent_tooltip", ""),
                context_obj.get("root_label", ""), context_obj.get("root_tooltip", ""),
            )
    except Exception as e:
...
ss_story_key = f"sparkmap_story_{topic}" # New key for the generated story
...
if ss_key not in st.session_state:
    ...
    with st.spinner("Generating SparkMap... This may take a moment."):
        try:
            # This part remains the same, using OpenAI to generate the map
            response = client.responses.create(
                model="gpt-4.1",
            ...
            st.error(f"Failed to generate SparkMap from the OpenAI model: {e}")
            st.stop()
    ...
    st.session_state[ss_txt_key] = text_file_content.encode("utf-8")
    st.session_state[ss_time_key] = t1 - t0
    st.session_state[ss_story_key] = None # Initialize story as None

# Retrieve from session state to display
mindmap_html = st.session_state[ss_key]
citations = st.session_state[ss_cit_key]
html_file = st.session_state[ss_html_key]
txt_file = st.session_state.get(ss_txt_key) # Use .get for safety
elapsed = st.session_state[ss_time_key]
story = st.session_state.get(ss_story_key)
...
with col1:
    st.download_button(
        label="Download SparkMap as HTML", data=html_file,
        file_name=f"{safe_filename}_BubbleDive_SparkMap.html", mime="text/html", use_container_width=True
    )
with col2:
    st.download_button(
        label="Download Outline as TXT", data=txt_file,
        file_name=f"{safe_filename}_BubbleDive_Outline.txt", mime="text/plain", use_container_width=True
    )
with col3:
    if st.button("Generate Story with Claude", use_container_width=True):
        if txt_file:
            outline = txt_file.decode("utf-8")
            generated_story = generate_story_with_claude(outline, topic)
            if generated_story:
                st.session_state[ss_story_key] = generated_story
                st.rerun() # Rerun to display the story and download button
        else:
            st.warning("Could not find the text outline to generate a story.")

# --- Display and Download Story ---
if story:
    st.markdown("### Your Story by Claude")
    with st.expander("Click to read the story", expanded=True):
        st.markdown(story)
    
    st.download_button(
        label="Download Story as TXT",
        data=story.encode("utf-8"),
        file_name=f"{safe_filename}_BubbleDive_Story.txt",
        mime="text/plain"
    )
